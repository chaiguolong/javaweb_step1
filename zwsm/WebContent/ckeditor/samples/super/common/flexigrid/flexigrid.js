...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 title: false,
			 pageButtom:'',	//底部按钮
			 pagestat: '显示从 {from} 到 {to} 条，共 {total} 条记录',
			 pagetext: '第',
			 outof: '页/ 共',
			 outend: '页',
			 findtext: '快速检索',
			 showFindDiv: false,//是否自动显示查询条件框
			 procmsg: '服务器查询中，请稍等 ...',
			 query: '',
			 qtype: '',
			 nomsg: '未查询到任何记录...',
		...
		var g = {
			...
			populate: function () { //get latest data
				...
				if (p.params)
					{	//是否带有参数的查询，若有每次查询时，重新从页面获取查询条件
						for(var n = 0; n < p.params.length; n ++)
						{	
							p.params[n].value = $('#' + p.params[n].name).val();
						}
					...
					$.ajax({
					   ...
					   data: param,
					   dataType: p.dataType,
					   success: function(data){g.addData(data);},//设置返回值
						...
						function ()
							{
									...
									if (p.sortname==$(pth).attr('abbr')&&p.sortname) 
									 ...
									 $(tdDiv).css({textAlign:pth.talign,width: $('div:first',pth)[0].style.width});
									 ...
									 if (this.innerHTML == '' || this.innerHTML == null) this.innerHTML = '&nbsp;';
			...
			for (i=0;i<p.colModel.length;i++)
				{
					...
					if (cm.talign)
						th.talign = cm.talign;	//2011-6-9 扩展BODY里可以调整数据位置
					else th.talign = 'center';
		...
		$(g.bDiv).after(g.pDiv);
		var html = ' <div class="pGroup"> <div class="pFirst pButton"><span></span></div><div class="pPrev pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">'+p.pagetext+' <input type="text" size="4" value="1" /> '+p.outof+' <span> 1 </span>' + p.outend + '</span></div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pNext pButton"><span></span></div><div class="pLast pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pReload pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pPageStat"></span></div>';
		...
		if ($.browser.msie&&$.browser.version<7) $('.pButton',g.pDiv).hover(function(){$(this).addClass('pBtnOver');},function(){$(this).removeClass('pBtnOver');});
			...
			$('.pDiv2',g.pDiv).prepend("<div class='pGroup'>" + p.pageButtom + "<select name='rp'>"+opt+"</select></div> <div class='btnseparator'></div>");
		...
		if (p.searchitems)
			{
				...
				$(g.sDiv).append("<div class='sDiv2'>"+p.findtext+"&nbsp;&nbsp;<select name='qtype'>"+sopt+"</select>&nbsp;&nbsp;<input type='text' size='30' name='q' class='qsbox' /> <!--input type='button' value='Clear' /--></div>");
	...
	//选中某一行的数据，将第一个值加载到数组中(将ID放在第一个，然后hide)，返回数组
	//主要用于批量操作
	$.fn.whichSelect = function(p) {
		var sa = new Array();
		//直接获取选中的对象
		$('.bDiv tbody tr.trSelected').each(function (){
			//将中的第一个ID加载到数组中
			sa.push($(this).find('div').eq(0).html());
		});
		return sa; 
	};
	//--end whichSelect
	...
	//判断当前table显示的数据长度
	$.fn.dataLength = function(p) {
		var i = 0;
		i = $('.bDiv tbody tr').size();
		return i;
	};
	
	//当前table显示的ID列表
	$.fn.getAllFlexHiddenID = function(p) {
		var sa = new Array();
		$('.bDiv tbody tr').each(function (){
			//将中的第一个ID加载到数组中
			sa.push($(this).find('div').eq(0).html());
		});
		return sa; 
	...
	/*
	$.fn.changedPageByPageNo = function(pageNo){
