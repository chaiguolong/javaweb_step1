...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<script type="text/javascript" src="/super/common/flexigrid/flexigrid.js"></script>
		...
		$(document).ready(function(){
			...
			$('#onefile').uploadify({
				...
				onUploadSuccess:function(file, data, response){//当每个文件上传完成后的操作
					var r = eval('(' + data + ')');	
					var times = (new Date()).getTime() + parseInt(100*Math.random());
					$('#onefileList').prepend('<li id="' + times + '"><img src="/' + r.path + '" title="' + r.path + '" style="max-height:550px; max-width:750px;"/><a href="javascript:removeTempFile(\'' + times + '\',\'' + r.path + '\',\'newsContent_thumbnail\');"><img src="/super/common/uploadify/uploadify-cancel.png" title="删除" /></a></li>');
		...
		function openCategory(){
			...
			$('body').append('<div id="treeDialog" title="资讯栏目"><div id="treeCategory" class="ztree"></div></div>');
			$("#treeDialog").dialog({
				position: [$("body").width()-350, 65],
				bgiframe: true,
				height: 450,
				width:320,
				modal: false,
			...
			$.ajax({
				url:'/esf/NewsContent_getZtreeCategory.jspx',
				type: 'get',dataType:'json',cache: false,async: false,
				error: function(xhr){
					exceptionAlert();
				},
				success: function(data){	
					treeObj = $.fn.zTree.init($("#treeCategory"), setting, data);
					if($.trim($('#newsContent_categoryids').val()) != ''){
						var _checked = $.trim($('#newsContent_categoryids').val()).split(',');
						for(var i in _checked){
							treeObj.checkNode(treeObj.getNodeByParam("id", $.trim(_checked[i]), null), true, false);
						}
					}
				}
			});
		...
		function doback(){
			if("${mode}" == "1"){
				window.location.href = "/esf/NewsContent_toContentLista.jspx?page=" + $('#page').val();
			}
			else if("${mode}" == "2"){
				window.location.href = "/esf/NewsContent_toContentListb.jspx?page=" + $('#page').val();
			}
			else if("${mode}" == "3"){
				window.location.href = "/esf/NewsContent_toContentListc.jspx?page=" + $('#page').val();
			}
			else if("${mode}" == "4"){
				window.location.href = "/esf/NewsContent_toContentListd.jspx?page=" + $('#page').val();
			}
			else{
				window.location.href = "/esf/NewsContent_toContentList.jspx?page=" + $('#page').val();
			}
		}
		...
		function doSubmit(state){
			...
			if(nodes.length > 0){
				...
				for(var i in nodes){
					categoryids.push(nodes[i].id);
				}
				$('#newsContent_categoryids').val(categoryids);
			}
			else {
				parent.showAlert("请选择资讯栏目！", "alert");
				return ;
			}
			...
			if($("#infoForm").validationEngine('validate')){
				...
				var saveOptions={
					target: '#infoForm',
					url:"/esf/NewsContent_saveContentInfo.jspx", type: 'post', dataType: 'html', cache:false, async:false,
	...
	<body class="tbody">
		...
		<div class="toolbg toolbgline toolheight nowrap" id="forContentWidth"
			...
			<div class="right">
				...
				<s:if test="%{newsContent == null || newsContent.releasemark == null || newsContent.releasemark == -3 || newsContent.releasemark == -2}">
					<input id="savebtn" type="button" class="wd1 btn" value="保存"
						onclick="javascript:doSubmit(1);"/>
				</s:if>
				<s:if test="%{newsContent.releasemark == -2 || newsContent.releasemark == -3}">
					<input id="submitbtn" type="button" class="wd2 btn" value="提交审核"
					onclick="javascript:doCheckSubmit(0);"/>
				</s:if>
				<s:if test="%{newsContent.releasemark == -1}">
					<input id="submitbtn" type="button" class="wd2 btn" value="审核通过"
					onclick="javascript:doCheckSubmit(0);"/>
					<input id="showbtn" type="button" class="wd3 btn" value="审核并发布"
					onclick="javascript:doCheckSubmit(1);"/>
				</s:if>
				<s:if test="%{newsContent.releasemark == 0}">
					<input id="showbtn" type="button" class="wd2 btn" value="确认发布"
					onclick="javascript:doCheckSubmit(1);"/>
				</s:if>
				<s:if test="%{newsContent.releasemark == 1}">
					<input id="showbtn" type="button" class="wd2 btn" value="确认修改"
					onclick="javascript:doCheckSubmit(1);"/>
				</s:if>
				<s:if test="%{newsContent.releasemark != null && newsContent.releasemark lt 1 && newsContent.releasemark gt -2}">
					<input id="gobackbtn" type="button" class="wd1 btn" value="退回"
						onclick="javascript:doCheckSubmit(-3);"/>
				</s:if>
			</div>
			<!-- 页面信息 -->
		...
		<form id="infoForm" name="infoForm">
			<div class="settingtable">
				<div class="settingtd settingDiv no_bt no_bb" style="margin: 0;">
					<div id="tabs">
						<ul>
							<li><a href="#tabs-0">资讯信息</a></li>
							<li><a href="#tabs-1">资讯内容</a></li>
						</ul>
						<div id="tabs-0">
							...
							<div class="settingDiv_r">
								...
								*	<a href="javascript:openCategory();"><font color="green">【选择栏目】</font></a>						
								</div>
							</div>
							<div class="settingDiv_l">
								外部链接URL：
