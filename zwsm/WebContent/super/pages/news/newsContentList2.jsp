...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
	...
	$(document).ready(function(){
		...
		//初始化当前页面cookie值
		if($.cookie('newContentListQuery2')){
			var query = $.evalJSON($.cookie('newContentListQuery2'));
			for(var i in query){
	...
	function saveQueryCookie(){
		var query = [
			...
			{ name : 'categoryid', value : $('#categoryid option:selected').val() }, 
		...
		$.cookie('newContentListQuery2', $.toJSON(query), {expires: 1});
	}
    ...
    function loadFlexigrid(){
    	saveQueryCookie();
    	$("#grid").flexigrid({
        	url : '/esf/NewsContent_getNewsContentList.jspx',
        	params:[
				{ name : 'starttime', value : $('#starttime').val() },
				{ name : 'endtime', value : $('#endtime').val() },  
				{ name : 'categoryid', value : $('#categoryid option:selected').val() }, 
				{ name : 'hotflag', value : $('#hotflag option:selected').val() }, 
				{ name : 'recommendflag', value : $('#recommendflag option:selected').val() }, 
				{ name : 'releasemark', value : $('#releasemark').val() }, 
				{ name : 'keywords', value : $('#keywords').val() }
			],
        	dataType : 'json',
           	colModel : [
	...
	//编辑
	function edit(id){
		window.location.href = "/esf/NewsContent_toContentInfo.jspx?page=" + $('.pcontrol input').val() + "&mode=2&newsContent.contentid=" + id;
	}
	...
	//预览
	function look(id){
		window.open('/info/news_newslook_' + id + '_' + ${sid} + '_1.htm');
	}
	</script>
  </head>
  <body class="tbody">
	<!-- 日志列表 -->
	<div class="txt_title">资讯中心 &gt; 待审核资讯列表   </div>
	<!-- 工具栏 -->
	<div class="toolbg toolbgline toolheight nowrap">
		...
		<div id="toolbarlist" class="nowrap left">
			<s:select name="categoryid" cssClass="validate[required]" 
				list="#request.newsCategoryList" headerKey="" headerValue="请选择..." 
				listKey="categoryid" listValue="typenamelevel+name" onchange="searchFlex();">
			</s:select>
			...
			<input type="hidden" id="releasemark" name="releasemark" value="-1"/>
